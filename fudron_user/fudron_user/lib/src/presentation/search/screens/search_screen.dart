  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      if (widget.initialQuery != null && widget.initialQuery!.isNotEmpty) {
        developer.log(
          '[VOICE] Search query passed: "${widget.initialQuery}"',
          name: 'VOICE',
        );
        developer.log(
          '[VOICE] Search started for query: "${widget.initialQuery}"',
          name: 'VOICE',
        );
  ...
  Widget build(BuildContext context) {
    ...
    var results = isVegOnly
        ? rawResults.where((r) {
            ...
            if (r.rawItem is RestaurantModel) {
              return (r.rawItem as RestaurantModel).isPureVeg;
            }
            if (r.rawItem is Store99Product) {
              return (r.rawItem as Store99Product).isVeg;
            }
            return true;
          }).toList()
        : rawResults;
    ...
    if (_categoryFilter == ResultCategoryFilter.dishes) {
      results = results
          .where((r) => r.type != SearchResultType.restaurant)
          .toList();
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      if (query != null &&
                          ...
                          context.mounted) {
                        developer.log(
                          '[VOICE] Search query passed: "$query"',
                          name: 'VOICE',
                        );
                        developer.log(
                          '[VOICE] Search started for query: "$query"',
                          name: 'VOICE',
                        );
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: chips.map((c) {
          final isSelected =
              state.mode == c.mode && _categoryFilter == c.filter;
          return Padding(
