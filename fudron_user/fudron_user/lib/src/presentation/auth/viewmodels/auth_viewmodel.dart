...
class AuthViewModel extends AsyncNotifier<UserModel?> {
  ...
  ///
  /// Returns false (with [lastError] set) if the server rejected the save, and
  /// restores the pre-edit user so the UI never keeps values the server refused.
  Future<bool> updateProfile({
    ...
    // Roll the optimistic update back. The server never took it, so leaving the
    // new values on screen would show a save that did not happen — they would
    // silently revert on the next profile refresh.
    lastError = result.message;
