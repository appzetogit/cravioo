...
import * as payLaterService from '../../user/services/payLater.service.js';
import {
...
async function applyCancellationRefund(order, { cancelledBy = 'system', refundAmount } = {}) {
  ...
  // Nothing was ever collected — same as cash — so cancelling just waives the
  // due back down instead of refunding anything. Checked before the 'paid'
  // gate below since pay_later orders sit at 'pay_later_due', never 'paid'.
  if (paymentMethod === 'pay_later') {
    if (paymentStatus === 'pay_later_due') {
      await payLaterService.waivePayLaterDue(order.userId, amount);
      order.payment.status = 'refunded';
      order.payment.refund = { status: 'processed', amount, processedAt: new Date() };
...
export async function createOrder(userId, dto) {
  try {
    // Applies regardless of the payment method chosen for THIS order — an
    // outstanding Pay Later due blocks checkout entirely until it's cleared.
    await payLaterService.assertNoOutstandingDue(userId);
    ...
    const payment = {
      method: paymentMethod,
      status: isCash ? "cod_pending" : isWallet ? "paid" : isPayLater ? "pay_later_due" : "created",
    ...
    if (isPayLater) {
      try {
        await payLaterService.chargePayLater(userId, order.pricing.total);
      } catch (err) {
        await FoodOrder.deleteOne({ _id: order._id });
        throw err;
      }
