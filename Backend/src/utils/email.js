...
export async function sendRestaurantApprovalEmail(to, payload = {}) {
    ...
    const subject = "Restaurant Approval Confirmed - Cravioo";
    const text = `Hi ${ownerName}, your restaurant "${restaurantName}" has been approved by admin. You can now log in and start accepting orders on Cravioo.`;
    const html = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 520px; margin: 0 auto; padding: 20px;">
  ...
  <p style="color: #999; font-size: 12px;">Cravioo Team</p>
</body>
</html>`;

    try {
        await trans.sendMail({
            from: typeof from === "string" && from.includes("<") ? from : `Cravioo <${from}>`,
            to: recipient,
            subject,
            text,
            html,
...
export async function sendDeliveryApprovalEmail(to, payload = {}) {
    ...
    const subject = "Delivery Partner Approval Confirmed - Cravioo";
    const text = `Hi ${partnerName}, your delivery partner profile has been approved by admin. You can now go online and start taking trips on Cravioo.`;
    const html = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
...
export async function sendSellerStatusEmail(to, payload = {}) {
    ...
    const body = asSafeText(payload.message, "There is an update on your Cravioo seller account.");
    const shopLine = shopName ? `Shop: ${shopName}` : "";
    const subject = heading;
    const text = [`Hi ${sellerName},`, shopLine, body, "Open the Cravioo seller app to see the latest status."]
        .filter(Boolean)
        .join("\n");
    const html = `
<!DOCTYPE html>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 520px; margin: 0 auto; padding: 20px;">
  ...
  <p>${body}</p>
  <p>Open the Cravioo seller app to see the latest status.</p>
  <hr style="border: none; border-top: 1px solid #eee; margin: 18px 0;">
  <p style="color: #999; font-size: 12px;">Cravioo Team</p>
</body>
</html>`;
...
export async function sendRestaurantRejectionEmail(to, payload = {}) {
    ...
    const subject = "Restaurant Application Update - Cravioo";
...
export async function sendDeliveryRejectionEmail(to, payload = {}) {
    ...
    const subject = "Delivery Partner Application Update - Cravioo";
