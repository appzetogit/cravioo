        ...
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <Input 
              ...
              className="pl-10 pr-10 h-11 bg-slate-100 dark:bg-zinc-800 border-none focus:ring-2 focus:ring-[#32C45A] rounded-xl"
            />
            {query && (
            ...
            <button 
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full transition-all ${isListening ? 'text-[#32C45A] scale-125 animate-pulse' : 'text-slate-400'}`}
            >
      ...
      <div className="max-w-3xl mx-auto p-4">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4">
              {categories.map((cat) => (
                ...
                >
                  <div className={`w-14 h-14 rounded-2xl mb-2 flex items-center justify-center overflow-hidden border-2 transition-all ${selectedCategoryId === cat._id ? 'border-[#32C45A] shadow-lg shadow-[#32C45A]/10' : 'border-transparent bg-white dark:bg-zinc-900'}`}>
                    {cat.image ? (
                      <img 
                  ...
                  <span className={`text-[11px] font-medium text-center line-clamp-1 ${selectedCategoryId === cat._id ? 'text-[#28A047]' : 'text-slate-600 dark:text-slate-400'}`}>
                    {cat.name}
                  </span>
                </button>
              ))}
        ...
        <AnimatePresence>
          {loading && (
            ...
            >
              <Loader2 className="w-8 h-8 text-[#32C45A] animate-spin mb-3" />
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            {results.dishes.length > 0 && (
              <section>
                <div className="flex items-center gap-2 mb-4">
                   <div className="w-1 h-5 bg-[#32C45A] rounded-full" />
                  ...
                  {results.dishes.map((r) => (
                    <Link to={`/food/user/restaurants/${r.slug || r._id}${r.matchedDishId ? `?dish=${r.matchedDishId}` : ''}`} key={r._id} className="flex gap-4 p-3 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 hover:shadow-md transition-shadow group">
                       ...
                       <div className="flex-1 min-w-0 flex flex-col justify-center">
                          <div className="text-[#32C45A] text-[10px] font-bold uppercase tracking-wider mb-1">
                          ...
                          <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-zinc-400 mt-1">
                             <div className="flex items-center gap-1">
                                <Star className="w-3 h-3 text-[#32C45A] fill-[#32C45A]" />
                  ...
                  {results.restaurants.map((r) => (
                    <Link to={`/food/user/restaurants/${r.restaurantId || r._id}`} key={r._id} className="block group">
                      <div className="relative rounded-3xl overflow-hidden aspect-[16/9] mb-3 bg-slate-200">
                        ...
                        {r.offer && (
                           <div className="absolute top-4 left-0 bg-[#32C45A] text-white text-[10px] font-bold px-3 py-1.5 rounded-r-lg shadow-lg flex items-center gap-1 tracking-tighter">
                      ...
                      <div className="flex items-center justify-between px-1">
                         ...
                         <div className="text-[10px] font-bold text-[#32C45A] bg-[#fff5f5] dark:bg-[#32C45A]/10 px-2 py-0.5 rounded-full uppercase tracking-wider">
                            Top Pick
                         </div>
                      </div>
                    </Link>
            ...
            {!loading && results.restaurants.length === 0 && results.dishes.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                 ...
                 <Button variant="outline" onClick={handleClear} className="mt-6 rounded-xl border-[#32C45A] text-[#32C45A] hover:bg-[#fff5f5]">
                    Clear all filters
                 </Button>
              </div>
            )}
